...
async function main() {
  ...
  const marcas = await Promise.all(['Samsung', 'Nike', 'Xiaomi', 'Apple', 'Adidas', 'Sony', 'LG', 'Lenovo', 'HP', 'Bosch'].map(m => 
    prisma.catMarca.create({ data: { nombre: m } })
  ));
  ...
  for (let sIdx = 0; sIdx < estados.length; sIdx++) {
    ...
    for (let i = 0; i < cantOrdenes; i++) {
      ...
      const fechaCreacion = DateTime.now().minus({ days: getRandomInt(1, 180) }).toJSDate();
      ...
      if (estadoNombre === 'ENTREGADA') {
        const fechaEntrega = DateTime.fromJSDate(fechaCreacion).plus({ days: getRandomInt(2, 5) }).toJSDate();
